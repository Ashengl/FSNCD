...
def merge_dataset(dataset1, dataset2):
    ...
    dataset1.samples = np.concatenate((dataset1.samples, dataset2.samples), axis=0)
...
def get_aircraft_datasets(train_transform, test_transform, train_classes=range(50), prop_train_labels=0.8,
    ...
    training_set = FGVCAircraft(root=aircraft_root, transform=train_transform, split='trainval')

    # Get test set for all classes
    test_set = FGVCAircraft(root=aircraft_root, transform=test_transform, split='test')
    ...
    train_dataset_labelled.label = [target_transform(i) for i in train_dataset_labelled.samples[:, 1].astype(np.int64)]
    if split_train_val:
        val_dataset_labelled.label = [target_transform(i) for i in val_dataset_labelled.samples[:, 1].astype(np.int64)]
    test_dataset.label = [target_transform(i) for i in test_dataset.samples[:, 1].astype(np.int64)]
